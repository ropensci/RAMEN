...
This minor version prepares RAMEN for submission to Bioconductor. It is mostly a
formatting and packaging release: apart from the change to the seed described
below, the analysis functions return the same results as in 2.1.2, and the
error and warning messages are unchanged.
...
RAMEN with GenomicRanges objects and Bioconductor: 
  ...
  a data frame. 
  - Downstream functions (i.e., `summarizeVML()`, `findCisSNPs()`, 
  ...
  "VML_df" to "VML". When appropriate, the output is now also a GRanges object.
  - `summarizeVML()` now outputs a matrix instead of a data frame. 
...
of the package, such as: 
  ...
  - Implemented internal helper functions to improve the readability of the code
  and improve its debugging. 
